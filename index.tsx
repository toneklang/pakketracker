...
// Initialize AI inside a helper to ensure process.env.API_KEY is ready
...
const loginToOutlook = (): Promise<string> => {
  return new Promise((resolve) => {
    setTimeout(() => resolve("MOCK_TOKEN"), 800);
  });
};
        ...
        <div className="flex justify-between items-end">
          ...
          {activeTab !== 'settings' && activeTab !== 'map' && (
            <div className="flex gap-2">
              <button onClick={handleSync} disabled={isProcessing} className="w-10 h-10 bg-gray-100 text-blue-500 rounded-full flex items-center justify-center shadow-md active:scale-95 transition-all">
        ...
        {activeTab === 'settings' ? (
          <div className="p-6">
            <div className="bg-white rounded-2xl p-4 shadow-sm mb-6">
              <p className="text-sm text-gray-500 mb-4">Data gemmes lokalt i browseren.</p>
              <button onClick={() => confirm('Slet alt data?') && setPackages([])} className="w-full py-3 bg-red-50 text-red-500 font-bold rounded-xl active:bg-red-100 transition-colors">Slet alt data</button>
            </div>
          </div>
        ) : activeTab === 'map' ? (
          <div className="flex flex-col items-center justify-center py-20 px-10 text-center text-gray-300">
            <svg className="w-16 h-16 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /></svg>
            <h3 className="text-lg font-bold text-gray-900 mb-1">Pakkeshops</h3>
            <p className="text-sm">Arbejder på at vise butikker direkte på kortet.</p>
          </div>
        ) : (
          <div className="px-6 pt-6">
            {showInput && (
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 mb-6 animate-in slide-in-from-top-4 duration-300">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1.5 text-blue-500 text-[11px] font-bold bg-blue-50 px-3 py-1.5 rounded-full">SCAN</button>
                ...
                <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} className="w-full h-24 p-3 bg-gray-50 border border-gray-100 rounded-xl text-sm mb-3 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Indsæt tekst her..." />
                <button 
                  onClick={async () => {
                    setIsProcessing(true);
                  ...
                  className="w-full py-3 bg-blue-500 text-white font-bold rounded-xl shadow-lg disabled:opacity-50 transition-all"
                >
            ...
            <div className="flex p-1 bg-gray-200/80 rounded-xl mb-6">
              <button onClick={() => setActiveTab('current')} className={`flex-1 py-1.5 text-sm font-semibold rounded-lg transition-all ${activeTab === 'current' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`}>Aktive ({packages.filter(p => p.status !== PackageStatus.PICKED_UP).length})</button>
              <button onClick={() => setActiveTab('history')} className={`flex-1 py-1.5 text-sm font-semibold rounded-lg transition-all ${activeTab === 'history' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500'}`}>Historik</button>
            </div>
